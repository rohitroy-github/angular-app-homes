...
@Component({
  ...
  template: `
    <article
      class="container mx-auto px-4 lg:px-16 py-10 flex flex-col lg:flex-row gap-10"
    >
      ...
      <section class="w-full lg:w-2/3">
        <img
          ...
          crossorigin
        />
        <h2 class="text-3xl sm:text-2xl font-bold mb-3">
        ...
        <h2 class="text-2xl sm:text-xl font-semibold text-blue-600 mb-3">
        ...
        <ul class="text-lg sm:text-base space-y-2">
          ...
          <li>
            🚗 Parking:
            <span class="font-semibold">{{
              housingLocation?.parking ? "Yes" : "No"
            }}</span>
          </li>
          <li>
            💰 Rent:
          ...
          <li>
            🛏️ Room Type:
            <span class="font-semibold">{{ housingLocation?.roomType }}</span>
          </li>
          <li>
            📍 Nearby Facilities:
            <span class="font-semibold">{{
              housingLocation?.nearby?.join(", ")
            }}</span>
          </li>
        </ul>
      </section>
      ...
      <section
        class="w-full lg:w-1/3 bg-gray-100 p-5 sm:p-4 rounded-lg shadow-md h-fit"
      >
        ...
        >
          <input
            ...
            class="input-field text-lg sm:text-base bg-white p-3 sm:p-2 rounded-lg w-full"
          />
          <input
            type="text"
          ...
          <button
            type="submit"
            class="bg-blue-500 text-white px-5 sm:px-4 py-2 rounded-lg hover:bg-blue-600 transition w-full cursor-pointer"
          >
            Apply Now
          </button>
        </form>

        <p *ngIf="message" class="text-blue-500 text-sm mt-3 text-center">
          {{ message }}
        </p>
      </section>
    </article>
